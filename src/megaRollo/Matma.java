...
public class Matma extends Number implements Comparable<Matma> {
    ...
    private void readObject(ObjectInputStream s)
         ...
         * the magnitude of a Matma is serialized as an array of bytes.
        ...
        // Validate signum
        if (sign < -1 || sign > 1) {
            String message = "Matma: Invalid signum value";
            if (fields.defaulted("signum"))
                message = "Matma: Signum not present in stream";
            throw new java.io.StreamCorruptedException(message);
        }
        int[] mag = stripLeadingZeroBytes(magnitude);
        if ((mag.length == 0) != (sign == 0)) {
            String message = "Matma: signum-magnitude mismatch";
            if (fields.defaulted("magnitude"))
                message = "Matma: Magnitude not present in stream";
            throw new java.io.StreamCorruptedException(message);
        }
        ...
        if (mag.length >= MAX_MAG_LENGTH) {
            try {
                ...
                throw new java.io.StreamCorruptedException("Matma: Out of the supported range");
            }
        }
    }
        ...
        static {
            try {
                ...
                signumOffset = unsafe.objectFieldOffset
                        (Matma.class.getDeclaredField("signum"));
                magOffset = unsafe.objectFieldOffset
                        (Matma.class.getDeclaredField("mag"));
            } catch (Exception ex) {
                throw new ExceptionInInitializerError(ex);
            }
        }

        static void putSign(Matma bi, int sign) {
            unsafe.putIntVolatile(bi, signumOffset, sign);
        }

        static void putMag(Matma bi, int[] magnitude) {
